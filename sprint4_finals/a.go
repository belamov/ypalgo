...
// -- ВРЕМЕННАЯ СЛОЖНОСТЬ --
// итоговая сложность O(N+M), где
//    N - количество документов,
//    M - количество поисковых запросов
// из этого времени:
//  - индекс строится за О(N), где N - количество документов. для каждого документа мы проходим по его словам, но максимум
//    слов в документе может быть 333 (длина документа не превосходит 1000, будут разные 2-буквенные слова), а значит количество слов в документе можем принять
//    за константу
//  - поисковые запросы выполняются за О(M), где M - количество поисковых запросов.
//    каждый отдельный запрос выполняется за О(1) - по факту асимптотика каждого поискового
//    запроса - О(H), где H - количество уникальных слов в запросе. но поскольку длина запроса
//    не превосходит 100 (из условия) - то уникальных слов может быть максимум 33 (если будут разные 2-буквенные слова),
//    поэтому мы можем считать, что сложность поискового запроса - О(33)~O(1)
//
// -- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
// программа будет потреблять O(K*L + L) памяти, где
...
//  K*L - используется для хранения поискового индекса в худшем случае, когда у нас много уникальных слов и эти слова встречаются в
//        большинстве документов
// также мы будем хранить О(P) памяти для хранения истории слов в поисковом запросе (чтобы не учитывать дубли), где P -
// количество уникальных слов в запросе. но посклольку их не может быть больше 33, то можно принять это за константу
func main() {
	scanner := makeScanner()
	n := readInt(scanner)
