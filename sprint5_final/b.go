...
// https://contest.yandex.ru/contest/24810/run-report/86325759/ - последнее ОК решение
...
// плюс мы повторно спускаемся по дереву от удаляемого элемента до заменяемого (чтобы удалить копию), но таких
// спусков всегда будет не больше, чем H, поэтому O(H+H)~O(H)
